...
(async function protectAdminPage() {
    try {
        ...
        sessionStorage.removeItem(ADMIN_AUTH_KEY);
        shivaSupabase.auth.signOut().finally(() => {
            window.location.replace("admin-login.html");
        });
    }
})();
...
const SERVICE_KEY = "shivaEventServices";
...
/*
   Jab existing service edit hoti hai,
   uploaded images (data:image...) ko yaha rakhenge.

   File input ko browser security ke karan
   dobara set nahi kiya ja sakta.

   Isliye purani uploaded images ko memory
   me preserve karke save karenge.
*/
...
function saveServices() {
    ...
    try {
        ...
        console.error("Could not save services:", error);
        alert("Photos bahut badi hain. Browser storage full ho sakta hai.");
        return false;
    }

    renderEverything();

    /* Mirror the complete service list to Supabase. */
    (async () => {
        try {
            const { data: oldRows, error: readError } = await shivaSupabase
                .from("services")
                .select("id");

            if (readError) throw readError;

            const keepIds = services
                .map(item => Number(item.id))
                .filter(Number.isFinite);

            for (const row of (oldRows || [])) {
                if (!keepIds.includes(Number(row.id))) {
                    const { error } = await shivaSupabase
                        .from("services")
                        .delete()
                        .eq("id", row.id);
            ...
            alert("Service browser me save ho gayi, lekin Supabase me save nahi hui.");
        }
    })();
...
function renderStats() {
    ...
    const newBookings =
        document.getElementById("newBookings");
        ...
        async () => {
            ...
            */
            ...
            if (editId.value) {
                ...
                /* Edit mode: new selected files replace old photos in preview. */
                renderPreview(
                    newImages.length
                        ? newImages
    ...
    imageUrls.addEventListener(
        ...
        async () => {
            ...
            if (editId.value) {
                ...
                /* URL photos are also treated as replacement photos in edit mode. */
                let selectedImages = [];
                ...
                else if (urlImages.length) {

                    renderPreview(urlImages);

                }
                else {

                    renderPreview(editingExistingImages);
...
const STORAGE_BUCKET = "shiva-event-images";
...
async function uploadFilesToStorage(fileList, serviceId) {
    ...
    for (const file of files) {
        ...
        // Keep individual uploads reasonably small for the free plan.
        if (file.size > 10 * 1024 * 1024) {
            throw new Error(`Image is larger than 10 MB: ${file.name}`);
        }

        const path = `services/${serviceId}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}-${safeStorageName(file.name)}`;

        const { error: uploadError } = await shivaSupabase.storage
            .from(STORAGE_BUCKET)
            .upload(path, file, {
                cacheControl: "3600",
                upsert: false,
        ...
        const { data: publicData } = shivaSupabase.storage
            .from(STORAGE_BUCKET)
            .getPublicUrl(path);
        ...
        async event => {
            ...
            try {
                ...
                if (
                    ...
                    /*
                       REAL UPLOAD: selected files go directly to
                       Supabase Storage. We store only their public URLs
                       in the services table, never base64 image data.
                    */
                    uploadedImages =
                ...
                if (isEditing) {
                    ...
                    /*
                       EDIT MODE PHOTO REPLACEMENT

                       Nayi photo upload karne par purani photos
                       append nahi hongi. Nayi uploaded photos
                       purani photos ko replace karengi.

                       Uploaded files are now Supabase Storage URLs.

                       Agar nayi file select nahi ki gayi hai,
                       URL textarea ki photos use hongi; aur agar
                       URL bhi nahi hai to existing photos rahengi.
                    */
                    ...
                    if (uploadedImages.length > 0) {
                        ...
                        finalImages = editingExistingImages;
                ...
                alert(
                    "Service save nahi ho paayi. Console check karo."
                );
...
function editService(id) {
    ...
    /*
       Data URLs textarea me nahi jayengi
       because they can be huge.

       External URLs textarea me rahengi.
    */
...
function markContacted(id) {
    ...
    shivaSupabase
        ...
        .eq("id", id)
        .then(({ error }) => {
            if (error) console.error("Booking status save error:", error);
        });
    ...
    clearBookingsButton.addEventListener(
        ...
        () => {
            ...
            shivaSupabase
                ...
                .neq("id", 0)
                .then(({ error }) => {
                    if (error) console.error("Clear bookings error:", error);
                });
...
/* =========================================================
   ADMIN REVIEWS - CONFIRM / REJECT SYSTEM
   ========================================================= */
...
function loadAdminReviews() {
    try {
        ...
        if (Array.isArray(saved)) {
            const normalized = saved.map(review => ({
                ...review,
        ...
        console.warn("Admin review loading error:", error);
    }
    return [];
}
...
function updateReviewNotification() {
    const reviews = loadAdminReviews();
    const pending = reviews.filter(review => review.status === "pending");

    if (pendingReviewBadge) {
        pendingReviewBadge.textContent = pending.length;
        pendingReviewBadge.classList.toggle("show", pending.length > 0);
    }

    if (reviewNotification) {
        reviewNotification.hidden = pending.length === 0;
        reviewNotification.classList.toggle("show", pending.length > 0);
    }

    if (reviewsTabBtn) {
        reviewsTabBtn.classList.toggle("has-pending", pending.length > 0);
        reviewsTabBtn.title = pending.length
...
function renderAdminReviews() {
    ...
    if (!reviews.length) {
        adminReviewList.innerHTML = `<div class="empty-state">No customer reviews yet.</div>`;
    ...
    reviews.slice().reverse().forEach(review => {
        ...
        row.className = `admin-review-row review-status-${review.status || "approved"}`;
        ...
        row.innerHTML = `
            <div class="admin-review-avatar">
                ${review.photo
                    ? `<img src="${escapeHTML(review.photo)}" alt="">`
                    : escapeHTML((review.name || "C").charAt(0).toUpperCase())}
            </div>

            <div class="admin-review-info">
                <div class="admin-review-name-line">
                    <strong>${escapeHTML(review.name)}</strong>
                    <span class="review-status-pill ${status}">${statusLabel}</span>
                </div>
                <div class="admin-review-stars">${"★".repeat(rating)}${"☆".repeat(5-rating)}</div>
                <small>${escapeHTML(review.date || "")}</small>
                <p>${escapeHTML(review.text)}</p>

                ${status === "pending" ? `
                    <div class="review-approval-actions">
                        <button class="review-confirm-btn" type="button" data-confirm-review="${review.id}">
                            <i class="fa-solid fa-check"></i> Confirm & Publish
                        </button>
                        <button class="review-reject-btn" type="button" data-reject-review="${review.id}">
                            <i class="fa-solid fa-xmark"></i> Reject
                        </button>
                    </div>
                ` : `
                    <div class="review-status-note">
                        ${status === "approved" ? "✓ Published on website" : "✕ Rejected — hidden from website"}
                    </div>
                `}
            </div>

            <button class="delete-btn" type="button" data-delete-review="${review.id}" title="Delete review">
                <i class="fa-solid fa-trash"></i>
            </button>
        `;
    ...
    adminReviewList.querySelectorAll("[data-confirm-review]").forEach(button => {
        button.addEventListener("click", () => updateReviewStatus(Number(button.dataset.confirmReview), "approved"));
    });

    adminReviewList.querySelectorAll("[data-reject-review]").forEach(button => {
        button.addEventListener("click", () => updateReviewStatus(Number(button.dataset.rejectReview), "rejected"));
    });

    adminReviewList.querySelectorAll("[data-delete-review]").forEach(button => {
        button.addEventListener("click", () => {
            const id = Number(button.dataset.deleteReview);
            if (!confirm("Delete this review permanently?")) return;
            const reviews = loadAdminReviews().filter(review => Number(review.id) !== id);
...
function updateReviewStatus(id, status) {
    const reviews = loadAdminReviews();
    const index = reviews.findIndex(review => Number(review.id) === Number(id));
    if (index === -1) return;
    ...
    /* Persist approval/rejection in Supabase. */
    shivaSupabase
        .from("reviews")
        .update({
            status
        ...
        .eq("id", id)
        .then(({ error }) => {
            if (error) console.error("Review status save error:", error);
        });

    if (status === "approved") {
        alert("Review confirmed and published on the website.");
    } else {
        alert("Review rejected. It will stay hidden from the website.");
    }
}

if (clearReviewsButton) {
    clearReviewsButton.addEventListener("click", () => {
        if (!confirm("Delete all customer reviews?")) return;
        ...
        shivaSupabase
            ...
            .neq("id", 0)
            .then(({ error }) => {
                if (error) console.error("Clear reviews error:", error);
            });
    });
}
...
/* =========================================================
   SUPABASE ADMIN REMOTE SYNC
   ========================================================= */

async function syncAdminServices() {
    try {
        const { data, error } = await shivaSupabase
            .from("services")
            .select("*")
        ...
        if (Array.isArray(data) && data.length) {
            services = normalizeServices(data.map(row => ({
                id: row.id,
            ...
            /* First launch: seed the existing demo/admin services online. */
            for (const item of services) {
                const { error: seedError } = await shivaSupabase
                    .from("services")
                    .upsert({
                        id: Number(item.id),
                        name: item.name,
        ...
        console.warn("Admin service sync skipped:", e);
    }
}
...
async function syncAdminBookings() {
    try {
        ...
        console.warn("Admin booking sync skipped:", e);
    }
}
...
async function syncAdminReviews() {
    try {
        ...
        console.warn("Admin review sync skipped:", e);
    }
}
